...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='long_progressions_midi',
                        help='输出目录 (默认: long_progressions_midi)')
    ...
    for i in range(args.num_progressions):
        ...
        folder_name = f"long_prog_{i+1:03d}_satb_{key_name}_{progression_str}"
        ...
        # 为每个声部创建MIDI文件
        create_midi_files_per_voice(progression, folder_path, args.tempo, args.chord_duration)
        
        print(f"[{i+1:2d}/{args.num_progressions}] ✓ {key_name:15} | {len(progression)}和弦 | {folder_name}/")
    
    print()
    print("="*70)
    print(f"✓ 完成! 共生成 {args.num_progressions} 个和弦进行文件夹")
    print(f"  输出目录: {output_dir}")
