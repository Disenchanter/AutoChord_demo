...
from train_chord_recognition import ChordCNN, ChordDataset, LabelExtractor
...
def main():
    parser = argparse.ArgumentParser(description='批量测试和弦识别模型')
...
if __name__ == '__main__':
    ...
    import glob
    import os
    model_dir = 'models_full_stft'
    mapping_dir = 'models_full_stft'
    # 查找最新模型
    model_files = sorted(glob.glob(os.path.join(model_dir, 'chord_model_full_*.pth')))
    mapping_files = sorted(glob.glob(os.path.join(mapping_dir, 'label_mappings_full_*.json')))
    if model_files and mapping_files:
